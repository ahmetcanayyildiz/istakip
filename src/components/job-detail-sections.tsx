        ...
        {collections.map((collection) => (
          <div key={collection.id} className="px-4 py-3.5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs text-foreground-muted tabular-nums">{formatDate(collection.date)}</p>
                <p className="mt-1 text-sm text-foreground-secondary">{collection.method}</p>
              </div>
    ...
    <section className="overflow-hidden rounded-lg border border-ui-border bg-surface shadow-xs">
      ...
      {quote ? (
        <div className="p-5">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-4 border-t border-ui-border-subtle pt-4 text-sm">
            <div><dt className="text-xs text-foreground-muted">Teklif Tarihi</dt><dd className="mt-1 text-foreground tabular-nums">{quote.createdAt}</dd></div>
            <div><dt className="text-xs text-foreground-muted">KDV Dahil Toplam</dt><dd className="mt-1 font-medium text-foreground tabular-nums">{formatCurrency(calculateQuoteTotals(quote).grandTotal)}</dd></div>
          </dl>
        </div>
      ) : (
